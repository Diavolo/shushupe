...
class PostsByTagListView(ListView):
    ...
    def get(self, *args, **kwargs):
        if not self.kwargs["tag_slug"].islower():
            return redirect("core:post-list-by-tag", self.kwargs["tag_slug"].lower())
...
class PageOrCategoryView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        page = (
            ...
            .prefetch_related("tags")
            .filter(slug=self.kwargs["page_slug"])
        )
        if page.count() != 0:
            return render(request, "core/page_detail.html", {"page": page.first()})
        category = get_object_or_404(Category, slug=self.kwargs["page_slug"])

        context = dict()
        context["post_list"] = Entry.get_published_article_list().filter(
...
class ArticleDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["previous_article"] = (
            Entry.get_published_article_list()
